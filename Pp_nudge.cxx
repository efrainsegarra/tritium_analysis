...
int main( int argc, char ** argv){
	...
	TH1D * hP4_a = new TH1D("hP4_a","p_{e',expected} - p_{e',measured}",10000,-2,2);	
	TH1D * hP4_b = new TH1D("hP4_b","p_{e',expected} - p_{e',measured}",10000,-2,2);	
	TH1D * hP4_c = new TH1D("hP4_c","p_{e',expected} - p_{e',measured}",10000,-2,2);	
	...
	double a,b,c;
	double fa,fb,fc;
	double del_p4_a, del_p4_b, del_p4_c;
	a = -100./1000.;
	b =  100./1000.;
	...
	for (int i = 0 ; i < nEvents ; i++){
		...
		double p4_exp = pow( pow( kinEbeam + mP - (L_mom - 0.000271386) , 2)	- pow(mP,2) , 0.5);
		del_p4_a = p4_exp  -  (kinMomR + a) * (1+R_delta);
		del_p4_b = p4_exp  -  (kinMomR + b) * (1+R_delta);
	...
	f1_a->SetParameter(0,hP4_a->GetMaximum());
	f1_a->SetParameter(1,hP4_a->GetMean());
	f1_a->SetParameter(2,hP4_a->GetRMS());
	f1_a->SetRange( hP4_a->GetMean() - 3*hP4_a->GetRMS()  , hP4_a->GetMean() + 0.01*hP4_a->GetRMS()  );
	f1_b->SetParameter(0,hP4_b->GetMaximum());
	f1_b->SetParameter(1,hP4_b->GetMean());
	f1_b->SetParameter(2,hP4_b->GetRMS());
	f1_b->SetRange( hP4_b->GetMean() - 3*hP4_b->GetRMS()  , hP4_b->GetMean() + 0.01*hP4_b->GetRMS()  );
	hP4_c->Fit("f1_c","QERS");
	hP4_a->Fit("f1_a","QERS");
	hP4_b->Fit("f1_b","QERS");
	...
	if( ( (fa < 0) && (fb > 0) ) || ( (fa > 0) && (fb < 0)   ) ){
		while( TMath::Abs(a-b) > 0.00001/1000){
			...
			for (int i = 0 ; i < nEvents ; i++){
				...
				double p4_exp_c = pow( pow( kinEbeam + mP - (L_mom - 0.000271386) , 2)	- pow(mP,2) , 0.5);
				del_p4_c = p4_exp_c  -  (kinMomR + c) * (1+R_delta);
				hP4_c->Fill( del_p4_c );
			}
			// Now we can get the mean of hP1
			...
			nCnt++;
			f1_c->SetParameter(0,hP4_c->GetMaximum());
			f1_c->SetParameter(1,hP4_c->GetMean());
			f1_c->SetParameter(2,hP4_c->GetRMS());
			f1_c->SetRange( hP4_c->GetMean() - 3*hP4_c->GetRMS()  , hP4_c->GetMean() + 0.01*hP4_c->GetRMS()  );
			hP4_c->Fit("f1_c","QERS");
